...
//协议中负载部分最大长度
#define CONFIG_COMM_FRAME_MAX_PAYLOAD       1024
...
public:
    ...
    void reallocMemory()
    {
        if (!isMaxCapacity())
        {
            ...
            {
                #ifdef WIN32
				LOG_ALWAYS("tid = %u, realloc to %d, cap:%d cap_max:%d front:%d rear:%d size:%d ", GetCurrentThreadId(), size_1, m_capacity, m_capacity_max, m_front, m_rear, m_size);
                #endif
                T * p = new T[size_1];
                if (p != nullptr)
                {
                ...
                #ifdef WIN32
				LOG_ALWAYS("tid = %u，realloc done, cap:%d cap_max:%d front:%d rear:%d size:%d ", GetCurrentThreadId(), m_capacity, m_capacity_max, m_front, m_rear, m_size);
                #endif
            }
        }
    }
...
struct ConfigCommFrame
{
	...
	uint8_t cmdArg;
	uint8_t sessionId;
	uint8_t sessionFinishFlag;
	uint8_t sessionResult;
	uint16_t pkgCnt;
	uint16_t pkgIdx;
	uint16_t dataLen;
	uint8_t data[CONFIG_COMM_FRAME_MAX_PAYLOAD];
};

//动态Json配置结构体
//typedef struct tagZCAN_DYNAMIC_CONFIG_JSON
//{
//	char    key[64];
//	char    value[64];
//}ZCAN_DYNAMIC_CONFIG_JSON;
...
public:
	...
	UINT   SetLINSubscribe(UINT chnIdx, PZCAN_LIN_SUBSCIBE_CFG pSend, UINT nMsgCount);
	UINT   SetLINPublish(UINT chnIdx, PZCAN_LIN_PUBLISH_CFG pSend, UINT nMsgCount);
    // UDS
    ZCAN_RET_STATUS UDS_Request(const ZCAN_UDS_REQUEST* req, ZCAN_UDS_RESPONSE* resp, BYTE* dataBuf, UINT dataBufSize);
    ZCAN_RET_STATUS UDS_Control(const ZCAN_UDS_CTRL_REQ *ctrl, ZCAN_UDS_CTRL_RESP* resp);
	ZCAN_RET_STATUS UDS_RequestEX(const ZCANUdsRequestDataObj* requestData, ZCAN_UDS_RESPONSE* resp, BYTE* dataBuf, UINT dataBufSize);
	ZCAN_RET_STATUS UDS_ControlEX(ZCAN_UDS_DATA_DEF dataType, const ZCAN_UDS_CTRL_REQ *ctrl, ZCAN_UDS_CTRL_RESP* resp);
public:
...
private:
    ...
    //PktType 0x52数据请求响应
    void HandleRequestDataEx(BYTE nSeq, PacketDataRequest& requestData);
    void HandleResponseDataEx(BYTE nSeq, PacketDataResponse& responseData);
    ...
    bool RequestDevData(PacketDataRequest& devRequest, PacketDataResponse& devResponse, bool autoProtocol = true);

    uint64_t GetDevRequestKey(BYTE pktSeq, uint32_t nTID);
    ...
    enum UDS_STATUS {
        ...
        UDS_STATUS_PARAM_ERR = 2,
    };
    bool GetUdsStatus(UINT reqId, UDS_STATUS &status);

    BYTE CreatePktRequestSeq();
    uint32_t CreatePktTID();

	// 设置动态配置跟持久配置
	bool addDynamicCfg(UINT chnIdx, const ZCAN_DYNAMIC_CONFIG_DATA* pObj);
	bool ApplyDynamicCfg(UINT chnIdx, UINT isPersist);// 是否是持久配置（即设备掉电保存配置）、TRUE-持久配置 FALSE-动态配置
	bool SendDynamicCfg(UINT chnIdx, std::string data, UINT isPersist);
	bool CheckDynamicCfgIsBaud(std::string cfg, std::string value, cJSON *obj);

	bool setDynamicCfg(UINT chnIdx, const ZCAN_DYNAMIC_CONFIG * pObj);//暂时只给设置滤波使用
	cJSON* _GenCANFDFilterCfgJson(const CANFD_FILTER_CFG& rConfig);
	cJSON* _GenCANFDFilterRuleItemJson(const CANFD_FILTER_RULE& rConfig);
	cJSON* _GenJsonByteArray(const BYTE* pByte, int nLen);

    // 设备是否支持扩展的请求应答包
    bool IsSupportRequestResponseEx();

	// 滤波
    bool ClearCANFilter(uint8_t nCANIndex);
    bool ApplyCANFilter();
...
private:
    ...
    std::mutex                              m_pktRequestSeqMutex;
    uint32_t                                m_pktTID{ 0 };          // pktType: 0x50 ,事务ID
    std::mutex                              m_pktTIDMutex;

    std::map<uint64_t, DevRequestResponse_PTR> m_pktRequestDataMap;
    ...
    BYTE                                    m_protocolVersion[2];   // 通讯协议版本号如 V1.19
	...
	//动态配置json数据
	std::map<std::string, std::string > m_dynamicCfgData;//key, value
	
	std::vector<CANFDNETFilterItem>     m_vecFilterItems[DEV_CHNL_COUNT_MAX];
};
