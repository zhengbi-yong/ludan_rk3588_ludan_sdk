...
//设置动态配置（json）
#define SETREF_ADD_DYNAMIC_BY_JSON              46  //添加动态配置，pData Pointer to tagZCAN_DYNAMIC_CONFIG_DATA, 用户需要申请内存，将内存地址和内存大小传填入ZCAN_RAW_DATA对应的字段中
...
#define SETREF_SET_DYNAMIC_BY_DATA              53  //设置动态配置(目前只给滤波用、走的是结构体下发数据)

//动态配置 持久配置 BEGIN
//网卡配置信息（还没支持）
//通用以太网配置
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_MAC "DYNAMIC_CONFIG_NIC_GENNIC_MAC"// 参数类型:string 网卡MAC地址，HEX字符串，如：”00:14 : 97 : 00 : 01 : 02”
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_DHCP "DYNAMIC_CONFIG_NIC_GENNIC_DHCP"// 参数类型:int DHCP是否打开，若打开则自动分配IP，即动态IP；0：关闭，1：打开，默认打开。
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_IP "DYNAMIC_CONFIG_NIC_GENNIC_IP"// 参数类型:string IP地址
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_MASK "DYNAMIC_CONFIG_NIC_GENNIC_MASK"// 参数类型:string 子网掩码
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_GATEWAY "DYNAMIC_CONFIG_NIC_GENNIC_GATEWAY"// 参数类型:string 网关IP
#define	ZCAN_DYNAMIC_CONFIG_NIC_GENNIC_ISMASTER "DYNAMIC_CONFIG_NIC_GENNIC_ISMASTER"// 车载以太网主从机设置（仅车载以太网有效）0：从机，1：主机
//车载以太网配置
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_MAC "DYNAMIC_CONFIG_NIC_CARNIC_MAC"// 参数类型:string 网卡MAC地址，HEX字符串，如：”00:14 : 97 : 00 : 01 : 02”
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_DHCP "DYNAMIC_CONFIG_NIC_CARNIC_DHCP"// 参数类型:int DHCP是否打开，若打开则自动分配IP，即动态IP；0：关闭，1：打开，默认打开。
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_IP "DYNAMIC_CONFIG_NIC_CARNIC_IP"// 参数类型:string IP地址
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_MASK "DYNAMIC_CONFIG_NIC_CARNIC_MASK"// 参数类型:string 子网掩码
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_GATEWAY "DYNAMIC_CONFIG_NIC_CARNIC_GATEWAY"// 参数类型:string 网关IP
#define	ZCAN_DYNAMIC_CONFIG_NIC_CARNIC_ISMASTER "DYNAMIC_CONFIG_NIC_CARNIC_ISMASTER"// 车载以太网主从机设置（仅车载以太网有效）0：从机，1：主机
//WIFI网卡信息配置
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_MAC "DYNAMIC_CONFIG_NIC_WIFINIC_MAC"// 参数类型:string 网卡MAC地址，HEX字符串，如：”00:14 : 97 : 00 : 01 : 02”
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_DHCP "DYNAMIC_CONFIG_NIC_WIFINIC_DHCP"// 参数类型:int DHCP是否打开，若打开则自动分配IP，即动态IP；0：关闭，1：打开，默认打开。
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_IP "DYNAMIC_CONFIG_NIC_WIFINIC_IP"// 参数类型:string IP地址
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_MASK "DYNAMIC_CONFIG_NIC_WIFINIC_MASK"// 参数类型:string 子网掩码
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_GATEWAY "DYNAMIC_CONFIG_NIC_WIFINIC_GATEWAY"// 参数类型:string 网关IP
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFINIC_ISMASTER "DYNAMIC_CONFIG_NIC_WIFINIC_ISMASTER"// 参数类型:int 车载以太网主从机设置（仅车载以太网有效）0：从机，1：主机
//WIFI模块配置（仅CANFDDTU-400/600EWGR有WIFI网卡）
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_ENWLAN "DYNAMIC_CONFIG_NIC_WIFIMODULE_ENWLAN"// 参数类型:int 用户一般无需操作;0：禁能 1：使能
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_SSID "DYNAMIC_CONFIG_NIC_WIFIMODULE_SSID"// 参数类型:string 设备作为 AP 热点模式时，为其无线设备的名称， SSID广播时可以被搜索到；设备作为 Station 客户端模式时，为其要连接的无线 AP 或者路由器的名称，名称长度最长为64字符（包括’\0’）
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_WPAPW "DYNAMIC_CONFIG_NIC_WIFIMODULEP_WPAPW"// 参数类型:string 设置模块作为 AP 时，其他设备需要输入密码；设置模块作为 Station 时，为连接 AP 或者路由器的密码。密码长度最长为64字符（包括’\0’），最少8字符（不包括’\0’）。
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_FREQ "DYNAMIC_CONFIG_NIC_WIFIMODULE_FREQ"// 参数类型:string 只支持2.4G，不可选
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_CHANNEL "DYNAMIC_CONFIG_NIC_WIFIMODULE_CHANNEL"// 参数类型:int 工作信道，范围： 1~13
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_WLANMODE "DYNAMIC_CONFIG_NIC_WIFIMODULE_WLANMODE"// 参数类型:string 0: AP, 做热点，可以被其他无线设备连接；1: Station, 做设备，连接路由器或者热点。
#define	ZCAN_DYNAMIC_CONFIG_NIC_WIFIMODULE_WPAMODE "DYNAMIC_CONFIG_NIC_WIFIMODULE_WPAMODE"// 参数类型:string 加密方式：wpa2_aes、wpa2、open(不加密)
//手机模块配置
#define	ZCAN_DYNAMIC_CONFIG_NIC_MOBLIE_ENABLE "DYNAMIC_CONFIG_NIC_MOBLIE_ENABLE"// 参数类型:int 使能移动网络，1-启用；0-禁用
//动态配置 持久配置 END
