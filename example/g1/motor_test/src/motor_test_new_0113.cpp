...
public:
    ...
    bool Initialize() {
        ...
        ZCAN_SetReference(ZCAN_CANFDNET_400U_TCP, 0, 0, CMD_DESIP, (void*)config_.zlg_ip.c_str());
    ...
    void ReceiveLoop() {
        ...
        while (receive_thread_running_) {
            ...
            for (uint32_t i = 0; i < received; i++) {
                ...
                uint32_t can_id = canfd_data.frame.can_id & 0x7FF;
                uint8_t len = canfd_data.frame.len;

                // 调试打印
                std::cout << "[DEBUG] CAN frame: CH=" << (int)receive_buffer[i].chnl
                          << ", ID=" << can_id << ", len=" << (int)len;
                if (len >= 8) {
