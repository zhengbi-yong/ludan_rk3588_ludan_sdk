...
const uint8_t PACKET_TYPE_REQUEST_RESPONSE_EX = 0x52; // 客户端发起请求，设备端回应（扩展格式）
...
#pragma pack(push,1)
#endif
typedef struct _LINTxInfo
{
	union
	{
		struct
		{
			USHORT    bSendMode : 2;      //Bit0~1: 发送方式： 0–订阅数据（此时根据报文长度 Length接收数据）1-从机应答数据 2-发送主机请求、仅发送报头 3、主机发送请求及数据（报头+从机数据）
			USHORT    bChkSumMode : 2;    //Bit2~3, 校验方式，仅发送方式为0/1/3有效
			USHORT    nReserved : 12;     //bit4-15, 保留，为0
		}unionLinTx;
		USHORT LinTx;
	};
#ifdef _WIN32
...
#pragma pack(pop)
#else
} __attribute__((aligned(1), packed)) LINTxInfo;
#endif

#ifdef _WIN32
#pragma pack(push,1)
#endif
typedef struct _LINRxInfo
{
	union
	{
		struct
		{
			USHORT    bDIR : 1;           //Bit0: 传输方向： 1-发送 0-接收 传输方向为从机任务方向
			USHORT    bERR : 1;           //Bit1： 0-正常报文 1-错误信息
			USHORT    bErrStage : 4;      //bit2-5：错误阶段，ERR为1时有效 0-总线空闲 1-Break段 2-同步段 3-ID段 4~12-数据错误（包括校验），表示接收哪个字节时出错，4表示字节0 15-未知
			USHORT    bErrReason : 4;     //bit6-9: 错误原因，ERR为1有效 0-接收超时 1-接收到不合法（不符合协议）数据，如ID校验错误、同步段错误、校验错误。此时报文中对应字段表示该错误实际值 2-接收到字节错误（停止位为显性） 3-Break段错误 15-未知错误
			USHORT    nReserved : 6;      //bit10~15：保留，为0
		}unionLinRx;
		USHORT LinRx;
	};
#ifdef _WIN32
...
#pragma pack(pop)
#else
} __attribute__((aligned(1), packed)) LINRxInfo;
#endif
...
const uint8_t PACKET_REQUEST_SET_CONFIG_PERSIST = 0x0c;        // 设置持久配置
const uint8_t PACKET_REQUEST_SET_SYNC_DEV_CLOCK = 0x12;        // 同步设备时间
...
const int PACKET_REQUEST_RESPONSE_EX_DATALEN_MIN = 5;  //const uint8_t PACKET_TYPE_REQUEST_RESPONSE_EX = 0x52 时，Data部分最短长度为5，包含 4Byte tid 和 1Byte result
...
public:
    ...
    size_t write(const uint8_t *data, size_t bytes)
    {
        ...
        size_t bytes_to_write = _min(bytes, capacity - size_);
    ...
    size_t read(uint8_t *data, size_t bytes)
    {
        ...
        size_t capacity = capacity_;
        size_t bytes_to_read = _min(bytes, size_);
...
public:
    ...
    static int  GetPacketDataRequestEx(const Packet& packet, PacketDataRequest& request);
    static int  GetPacketDataResponse(const Packet& packet, PacketDataResponse& response);
    static int  GetPacketDataResponseEx(const Packet& packet, PacketDataResponse& response);
	...
	static void PacketLIN2ZCANDataObj(const PacketDataLIN& rProtocalFrame, ZCANDataObj& rZCANDataObj);
	...
	static void ZLIN2PacketLIN(int nChnl, const ZCANLINData &zcanLINData, PacketDataLIN& pktLIN);
	static void ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_SUBSCIBE_CFG &zLINSubscibe, PacketDataLIN& pktLIN);
	static void ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_PUBLISH_CFG &zLINResponse, PacketDataLIN& pktLIN);
	//static void ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_SUBSCIBE_CFG &zLINSubscibe, PacketDataLIN& pktLIN);
...
public:
    ...
    static Packet     BuildPacketRequestEx(uint8_t nSeq, const PacketDataRequest& request);
    static Packet     BuildPacketEmulateResponse(uint8_t nSeq, const PacketDataRequest& requestData);
    static Packet     BuildPacketEmulateResponseEx(uint8_t nSeq, const PacketDataRequest& requestData);
	...
	//2022.11.12新加
	static VecPackets BuildPacketLIN(int nChnl, const PZCAN_LIN_SUBSCIBE_CFG pSend, ULONG len);
	static VecPackets BuildPacketLIN(int nChnl, const PZCAN_LIN_PUBLISH_CFG pSend, ULONG len);
