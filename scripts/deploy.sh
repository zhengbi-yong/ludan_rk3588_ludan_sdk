#!/bin/bash
# Jetson Deploy Script - ROS Bridge to RK3588
# 通过ROS bridge发送/lowcmd到RK3588的rt/lowcmd topic
...
USE_NETWORK="${USE_NETWORK:-false}"        # 禁用UDP网络传输
USE_DDS_BRIDGE="${USE_DDS_BRIDGE:-true}"  # 启用DDS桥接
USE_ROS_BRIDGE="${USE_ROS_BRIDGE:-true}"   # 启用ROS bridge
...
show_config() {
    echo "=================================================="
    echo "      Jetson ROS Bridge Deployment Configuration "
    echo "=================================================="
    echo "Target RK3588:      $RK3588_IP"
    ...
    echo "Network Mode:       $USE_NETWORK (UDP传输)"
    echo "DDS Bridge:         $USE_DDS_BRIDGE (直接DDS)"
    echo "ROS Bridge:         $USE_ROS_BRIDGE (ROS转发)"
    echo ""
    echo "通信路径:"
    echo "  Jetson ROS1 -> ROS Bridge -> RK3588 DDS rt/lowcmd"
    echo "=================================================="
    echo ""
}
...
check_environment() {
    ...
    if [ "$USE_ROS_BRIDGE" = "true" ]; then
        if [ ! -f "$(dirname $0)/../ros_integration/ros1_to_dds_bridge.py" ]; then
            log_error "ROS Bridge script not found!"
            exit 1
        fi
        log_success "ROS Bridge script found"
    fi

    # 设置可执行权限
    chmod +x "$(dirname $0)/jetson_lowcmd_publisher.py"
    if [ "$USE_ROS_BRIDGE" = "true" ]; then
        chmod +x "$(dirname $0)/../ros_integration/ros1_to_dds_bridge.py"
    fi
}
...
start_publisher() {
    ...
    log_info "Using ROS Bridge mode - publishing to /lowcmd ROS1 topic"
...
cleanup() {
    ...
    # 停止ROS Bridge
    if [ -f /tmp/ros_bridge.pid ]; then
        ROS_BRIDGE_PID=$(cat /tmp/ros_bridge.pid)
        if kill -0 $ROS_BRIDGE_PID 2>/dev/null; then
            kill $ROS_BRIDGE_PID 2>/dev/null || true
            log_success "ROS Bridge stopped"
        fi
        rm -f /tmp/ros_bridge.pid
    fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --no-ros-bridge      Disable ROS bridge (default: enabled)"
            echo "  --ros-master-uri     ROS Master URI (default: http://localhost:11311)"
            echo "  --help               Show this help"
            echo ""
            echo "Example:"
            echo "  $0                   # Start with ROS bridge (default)"
            echo "  $0 --no-ros-bridge   # Start without ROS bridge"
            echo "  $0 --no-network      # Disable UDP network transmission"
            exit 0
            ;;
        *)
