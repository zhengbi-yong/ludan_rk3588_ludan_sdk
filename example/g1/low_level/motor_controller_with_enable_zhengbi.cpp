...
public:
    ...
    static std::vector<uint8_t> BuildCanPacket(
        ...
        // 10. XOR Checksum: 字节1到字节29的异或（不包括包头和校验位本身）
        uint8_t checksum = 0;
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cout << "  -c, --channel <ch>      CAN channel 0-3 (auto-detected from port, overrides auto)" << std::endl;
        ...
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8000" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8003 -v" << std::endl;
        std::cout << "\n  # Port to CAN mapping (auto-detected):" << std::endl;
        std::cout << "  # 8000=CAN0, 8001=CAN1, 8002=CAN2, 8003=CAN3" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8000 --enable 1   # CAN0" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8001 --enable 1   # CAN1" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8002 --enable 1   # CAN2" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8003 --enable 1   # CAN3" << std::endl;
        std::cout << "\n  # Direct enable/disable mode" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8000 --disable 1" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8003 --disable 5" << std::endl;
        std::cout << "\n  # Enable all motors (1-10) on CAN2" << std::endl;
        std::cout << "  ./motor_controller_with_enable 192.168.1.5:8002 --enable 1-10" << std::endl;
        return 1;
    }
    ...
    // 从 tcp_endpoint 提取端口号，用于自动推导通道号
    size_t colon_pos = params.tcp_endpoint.find(':');
    if (colon_pos != std::string::npos) {
        int port = std::atoi(params.tcp_endpoint.substr(colon_pos + 1).c_str());
        // 端口映射: 8000->CAN0, 8001->CAN1, 8002->CAN2, 8003->CAN3
        if (port >= 8000 && port <= 8003) {
            params.channel = port - 8000;
            params.zlg_port = port;
        }
    }

    bool channel_overridden = false;  // 标记是否手动指定了通道
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (arg == "-v" || arg == "--verbose") {
            ...
            // 端口改变时自动更新通道号
            if (params.zlg_port >= 8000 && params.zlg_port <= 8003) {
                if (!channel_overridden) {
                    params.channel = params.zlg_port - 8000;
                }
            }
        }
        else if ((arg == "-c" || arg == "--channel") && i + 1 < argc) {
            params.channel = std::atoi(argv[++i]);
            channel_overridden = true;  // 标记为手动指定
            if (params.channel < 0 || params.channel > 3) {
                std::cerr << "Error: CAN channel must be 0-3 (got: " << params.channel << ")" << std::endl;
                return 1;
            }
        }
    ...
    std::cout << "ZLG Port: " << params.zlg_port << " -> CAN" << params.channel << std::endl;
    std::cout << "CAN Channel: " << params.channel << (channel_overridden ? " (manual)" : " (auto-detected)") << std::endl;
    ...
    if (params.direct_mode) {
        ...
        // 设置 ZLG 配置
        sender->SetConfig(params.zlg_ip, params.zlg_port, params.channel);
    ...
    try {
        ...
        // 设置 ZLG 配置
        bridge->SetZlgConfig(params.zlg_ip, params.zlg_port, params.channel);
