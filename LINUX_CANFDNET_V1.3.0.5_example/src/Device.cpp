...
void Device::ClearBuffer(UINT chnIdx)
{
    if (chnIdx < m_devChnlCount)
    {
		...
		m_frmMerge.clear();//2023.03.28新加清空合并接收缓存数据
...
bool Device::_StopAndClear()
{
	...
	//只有closedevice的时候才会清空 resetcan/resetlin不清空
    //m_bTxEcho = false;
    //m_bRecvMerge = false;
...
bool Device::InitLIN(UINT chnIdx, PZCAN_LIN_INIT_CONFIG pLINInitConfig)
{
    if (chnIdx < DEV_LIN_COUNT_MAX && pLINInitConfig != NULL)
    {
		...
		m_linConfig[chnIdx].nEnable = 0;
		m_linConfig[chnIdx].stFeature.rawVal = 0;
		//配置信息的chkSumMode跟报文信息chkSumMode不一致、此处进行转换适配
...
void Device::HandleLINFrame(const std::vector<PacketDataLIN>& vecFrame)
{
    if (m_bRecvMerge)
    {
        ...
        for (auto & pkt : vecFrame)
		{
			...
			if (!m_bitChnlStarted.test(nLINIndex))//lin通道没开则过滤掉数据
			{
				continue;
			}
            CPacketUtil::PacketData2ZCANDataObj(pkt, dataObj);
...
{
	UINT    nBaudVal;       //波特率值
	UINT    nTSEG1;         //波特率值对应的位定时值
	UINT    nTSEG2;
	UINT    nSJW;
	UINT    nBRP;
};

// USBCANFD800(以及相同时钟频率的设备)用的波特率参数
...
// CANFDDTU-300(以及相同时钟频率的设备)用的波特率参数
...
cJSON* Device::_GenCANFDFilterRuleItemJson(const CANFD_FILTER_RULE& rConfig)
{
	...
	//当同时接收ERR和CAN/CANFD数据时( rConfig.presentFlag.unionValue.bErr == 0)，
	//或者只接收错误帧时( rConfig.presentFlag.unionValue.bErr == 1 && rConfig.nErr != 0)
	//只需要通道参数即可，不再生成后续的配置
	if (!rConfig.presentFlag.unionValue.bErr ||
...
bool Device::ApplyCANFilter()
{
	...
	if (nCount > 0)
	{
		...
		cJSON_AddNumberToObject(filter, "Whitelist", 1);//默认是白名单
		...
		cJSON_free(pStr);
		cJSON_Delete(root);
		return SendDynamicCfg(0, reqParam, 0);//默认是动态配置
	}
	else
	{
		cJSON* root = cJSON_CreateObject();
