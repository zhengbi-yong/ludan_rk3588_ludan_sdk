...
bool CPacketUtil::IsValidPacketHead(const Packet& packet)
{
    ...
    if (size >= PACKET_HEAD_LEN)
    {
        ...
        if (pHead && pHead->nPacketHead == PACKET_HEADER)
		{
            ...
            case PACKET_TYPE_REQUEST_RESPONSE_EX:
                bRet = nPacketDataLen >= 0;
                break;
            default:
                bRet = false;
...
UINT64 CPacketUtil::GetNowTimeStampAsMicrosecond()
{
	...
	//static LARGE_INTEGER nBeginTime = nTime;
...
int  CPacketUtil::GetPacketDataRequest(const Packet& packet, PacketDataRequest& request)
{
    ...
    if (PACKET_TYPE_REQUEST_RESPONSE == pktType && pktTypeParam.unionValue.bResponse == 0)
    {
            ...
            nDataLen + PACKET_HEAD_LEN + 1 == packet.size())  //crc 1 uint8_t
        {
            ...
            request.nTID = vecData[0];
            request.nRequest = vecData[1];
...
int  CPacketUtil::GetPacketDataRequestEx(const Packet& packet, PacketDataRequest& request)
{
    int nRet = 0;
    uint8_t pktType = GetPacketFrameType(packet);
    PacketTypeRequestResponseParam pktTypeParam;
    pktTypeParam.rawValue = CPacketUtil::GetPacketFrameTypeParam(packet);
    if (PACKET_TYPE_REQUEST_RESPONSE_EX == pktType && pktTypeParam.unionValue.bResponse == 0)
    {
        int nDataLen = GetPacketDataLen(packet);
        if (nDataLen >= PACKET_REQUEST_RESPONSE_EX_DATALEN_MIN &&
            nDataLen + PACKET_HEAD_LEN + 1 == packet.size())  //crc 1 uint8_t
        {
            FrameData vecData;
            auto itrDatabegin = packet.begin() + PACKET_HEAD_LEN;
            vecData.assign(itrDatabegin, itrDatabegin + nDataLen);
            uint32_t tid = 0;
            memcpy(&tid, &vecData[0], 4);
            request.nTID = cc_ntohl(tid);
            request.nRequest = vecData[4];
            request.vecData.assign(vecData.begin() + 5, vecData.begin() + nDataLen);  // 5byte tid and result
            nRet = 1;
        }
        else
        {
            LOG_ERR("Packet GetPacketDataRequestEx Lenth Invalid, pkt.len=%d, pkt.size=%d\n", nDataLen, packet.size());
        }
    }
    else
    {
        LOG_ERR("GetPacketFrameType GetPacketDataRequestEx Mismatch!\n");
    }
    return nRet;
}
...
int  CPacketUtil::GetPacketDataResponse(const Packet& packet, PacketDataResponse& response)
{
    ...
    if (PACKET_TYPE_REQUEST_RESPONSE == pktType && pktTypeParam.unionValue.bResponse)
    {
            ...
            nDataLen + PACKET_HEAD_LEN + 1 == packet.size())  //crc 1 uint8_t
        {
            ...
            auto itrDatabegin = packet.begin() + PACKET_HEAD_LEN;
            vecData.assign(itrDatabegin, itrDatabegin + nDataLen);
            response.nTID = vecData[0];
            response.nResult = vecData[1];
...
int  CPacketUtil::GetPacketDataResponseEx(const Packet& packet, PacketDataResponse& response)
{
    int nRet = 0;
    uint8_t pktType = GetPacketFrameType(packet);
    PacketTypeRequestResponseParam pktTypeParam;
    pktTypeParam.rawValue = CPacketUtil::GetPacketFrameTypeParam(packet);
    if (PACKET_TYPE_REQUEST_RESPONSE_EX == pktType && pktTypeParam.unionValue.bResponse)
    {
        int nDataLen = GetPacketDataLen(packet);
        if (nDataLen >= PACKET_REQUEST_RESPONSE_EX_DATALEN_MIN &&
            nDataLen + PACKET_HEAD_LEN + 1 == packet.size())  //crc 1 uint8_t
        {
            ...
            uint32_t tid = 0;
            memcpy(&tid, &vecData[0], 4);
            response.nTID = cc_ntohl(tid);
            response.nResult = vecData[4];
            response.vecData.assign(vecData.begin() + 5, vecData.begin() + nDataLen);  // 4byte tid and result
            nRet = 1;
        }
        else
        {
            LOG_ERR("Packet GetPacketDataResponseEx Lenth Invalid, pkt.len=%d pkt.size=%d\n", nDataLen, packet.size());
        }
    }
    else
    {
        LOG_ERR("GetPacketFrameType GetPacketDataResponseEx Mismatch!\n");
    }
    return nRet;
}
...
bool CPacketUtil::GetPacket(const FrameData& pktFrame, PacketDataLIN& pktLIN)
{
    if (pktFrame.size() == sizeof(pktLIN))
    {
		...
		pktLIN.frameInfo.linInfoRawVal = cc_ntohs(pNetOrder->frameInfo.linInfoRawVal);
...
void CPacketUtil::ZCANTx2CANFrame(const ZCAN_Transmit_Data& zTx, int nChnl, bool bTxEchoEnable, PacketDataCAN& pktFrame)
{
	...
	// 设置数据长度，不符合dlc规则补齐到符合dlc规则的长度
	if (zTx.frame.can_dlc <= 8) {
		pktFrame.canHead.nDataLen = zTx.frame.can_dlc;
	} else{
		pktFrame.canHead.nDataLen =8;
	}
...
void CPacketUtil::ZCANTx2CANFrame(const ZCAN_TransmitFD_Data& zTx, int nChnl, bool bTxEchoEnable, PacketDataCANFD& pktFrame)
{
	...
	if (zTx.frame.len <= 8) {
		pktFrame.canHead.nDataLen = zTx.frame.len;
	} else if (zTx.frame.len <= 24) {
		if (zTx.frame.len % 4 != 0) {
			pktFrame.canHead.nDataLen = ((zTx.frame.len / 4) + 1) * 4;
		} else{
			pktFrame.canHead.nDataLen = zTx.frame.len;
		}
	} else {
		if (zTx.frame.len % 16 != 0) {
			pktFrame.canHead.nDataLen = ((zTx.frame.len / 16) + 1) * 16;
		} else {
			pktFrame.canHead.nDataLen = zTx.frame.len;
		}
	}
...
bool CPacketUtil::ZCANDataObj2CANFrame(const ZCANDataObj& zTx, PacketDataCANFD& pktFrame)
{
	...
	if (zTx.data.zcanCANFDData.flag.unionVal.frameType==0) //等于0是CAN的
	{
		if (zTx.data.zcanCANFDData.frame.len <= 8) {
	...
	else		//CANFD
	{
		if (zTx.data.zcanCANFDData.frame.len <= 8) {
			pktFrame.canHead.nDataLen = zTx.data.zcanCANFDData.frame.len;
		} else if (zTx.data.zcanCANFDData.frame.len <= 24) {
			if (zTx.data.zcanCANFDData.frame.len % 4 != 0) {
				pktFrame.canHead.nDataLen = ((zTx.data.zcanCANFDData.frame.len / 4) + 1) * 4;
			} else{
				pktFrame.canHead.nDataLen = zTx.data.zcanCANFDData.frame.len;
			}
		} else {
			if (zTx.data.zcanCANFDData.frame.len % 16 != 0) {
				pktFrame.canHead.nDataLen = ((zTx.data.zcanCANFDData.frame.len / 16) + 1) * 16;
			} else {
				pktFrame.canHead.nDataLen =zTx.data.zcanCANFDData.frame.len;
			}
		}
...
void CPacketUtil::CANFrame2ZCANRx(const PacketDataCAN& pktFrame, ZCANDataObj& zRx)
{
    ...
    canfdData.frame.can_id = MAKE_CAN_ID(pktFrame.canHead.nID, pktFrame.canHead.frameInfo.unionVal.bEXT,
        pktFrame.canHead.frameInfo.unionVal.bRTR, pktFrame.canHead.frameInfo.unionVal.bERR);
    memcpy(canfdData.frame.data, pktFrame.canData, sizeof(pktFrame.canData));
...
void CPacketUtil::CANFrame2CANFDFrame(const PacketDataCAN& pktFrameCAN, PacketDataCANFD& pktFrameCANFD)
{
    ...
    memcpy(pktFrameCANFD.canData, pktFrameCAN.canData, sizeof(pktFrameCAN.canData));
}
...
bool CPacketUtil::CANErrorData2ZCANDataObj(const PacketDataCANHead& canHead, const uint8_t* data, ZCANDataObj& zcanDataObj)
{
	...
	errData.timeStamp = canHead.nTimestamp;
...
void CPacketUtil::DumpData(uint8_t* pData, int nLen)
{
    ...
    LOG_ALWAYS("Dump data:%d Bytes", nLen);
    std::string strLine;
    char tmpByte[4] = { 0 };
    int nLine = 0;
    ...
    LOG_ALWAYS("Dump data done, line count:%d", nLine);
}
...
void CPacketUtil::PacketLIN2ZLIN(const PacketDataLIN& pktLIN, ZCAN_LIN_MSG& zLINMsg, int* pChnl)
{
	memset(&zLINMsg, 0, sizeof(zLINMsg));
	zLINMsg.chnl = pktLIN.nChnl;
	if (!pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bERR)
	{
		zLINMsg.dataType = 0;
		ZCANLINData& zLIN = zLINMsg.data.zcanLINData;
		//zLIN.DataType = 1;
		zLIN.RxData.timeStamp = pktLIN.nTimestamp;
		zLIN.PID.rawVal = pktLIN.stMsgID.nRawID;
		zLIN.RxData.dataLen = pktLIN.nDataLen;
		zLIN.RxData.dir = pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bDIR;
		zLIN.RxData.chkSum = pktLIN.nChkSum;
		memcpy(zLIN.RxData.data, pktLIN.nData, 8);
	}
	else//错误帧
	{
		zLINMsg.dataType = 1;
		ZCANLINErrData &zLINErrData = zLINMsg.data.zcanLINErrData;
		zLINErrData.timeStamp = pktLIN.nTimestamp;
		zLINErrData.PID.rawVal = pktLIN.stMsgID.nRawID;
		zLINErrData.dataLen = pktLIN.nDataLen;
		zLINErrData.dir = pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bDIR;
		zLINErrData.chkSum = pktLIN.nChkSum;
		memcpy(zLINErrData.data, pktLIN.nData, 8);

		zLINErrData.errData.errStage = pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bErrStage;
		zLINErrData.errData.errReason = pktLIN.frameInfo.linInfo.linRxInfo.unionLinRx.bErrReason;
	}

	return;
}

void CPacketUtil::PacketLIN2ZCANDataObj(const PacketDataLIN& rProtocalFrame, ZCANDataObj& rZCANDataObj)
{
	memset(&rZCANDataObj, 0, sizeof(rZCANDataObj));
	rZCANDataObj.chnl = rProtocalFrame.nChnl;
	if (!rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bERR)
	{
		rZCANDataObj.dataType = ZCAN_DT_ZCAN_LIN_DATA;
		ZCANLINData& zLIN = rZCANDataObj.data.zcanLINData;
		//zLIN.DataType = 1;
		zLIN.RxData.timeStamp = rProtocalFrame.nTimestamp;
		zLIN.PID.rawVal = rProtocalFrame.stMsgID.nRawID;
		zLIN.RxData.dataLen = rProtocalFrame.nDataLen;
		zLIN.RxData.dir = rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bDIR;
		zLIN.RxData.chkSum = rProtocalFrame.nChkSum;
		memcpy(zLIN.RxData.data, rProtocalFrame.nData, 8);
	}
	else//错误帧
	{
		rZCANDataObj.dataType = ZCAN_DT_ZCAN_LIN_ERROR_DATA;
		ZCANLINErrData &zLINErrData = rZCANDataObj.data.zcanLINErrData;
		zLINErrData.timeStamp = rProtocalFrame.nTimestamp;
		zLINErrData.PID.rawVal = rProtocalFrame.stMsgID.nRawID;
		zLINErrData.dataLen = rProtocalFrame.nDataLen;
		zLINErrData.dir = rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bDIR;
		zLINErrData.chkSum = rProtocalFrame.nChkSum;
		memcpy(zLINErrData.data, rProtocalFrame.nData, 8);

		zLINErrData.errData.errStage = rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bErrStage;
		zLINErrData.errData.errReason = rProtocalFrame.frameInfo.linInfo.linRxInfo.unionLinRx.bErrReason;
	}
...
bool CPacketUtil::ZCANDataObj2PacketLIN(const ZCANDataObj& zTx, PacketDataLIN& pktLIN)
{
	...
	//assert(zTx.data.zcanLINData.DataType == 0);
	memset(&pktLIN, 0, sizeof(pktLIN));
	pktLIN.nChnl = zTx.chnl;
	pktLIN.stMsgID.nRawID = zTx.data.zcanLINData.PID.rawVal;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bSendMode = LIN_SEND_HEADER;
    return true;
}

//发送头
void CPacketUtil::ZLIN2PacketLIN(int nChnl, const ZCANLINData &zcanLINData, PacketDataLIN& pktLIN)
{
	/*if (zcanLINData.DataType != 0)
	{
		return;
	}*/
	memset(&pktLIN, 0, sizeof(pktLIN));
	pktLIN.nChnl = nChnl;
	pktLIN.stMsgID.nRawID = zcanLINData.PID.rawVal;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bSendMode = LIN_SEND_HEADER;
}

//设置发布数据
void CPacketUtil::ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_PUBLISH_CFG &zLINResponse, PacketDataLIN& pktLIN)
{
	if (zLINResponse.chkSumMode >= AUTOMATIC || zLINResponse.ID > MAX_LIN_ID_COUNT
		|| zLINResponse.dataLen > MAX_LIN_DLC || zLINResponse.dataLen == 0)
	{
		return;
	}
	memset(&pktLIN, 0, sizeof(pktLIN));
	pktLIN.nChnl = nChnl;
	pktLIN.stMsgID.nRawID = zLINResponse.ID;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bSendMode = LIN_SEND_RESPONSE;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bChkSumMode = zLINResponse.chkSumMode;
	pktLIN.nDataLen = zLINResponse.dataLen;
	memcpy(pktLIN.nData, zLINResponse.data, 8);
}

//设置订阅
void CPacketUtil::ZLIN2PacketLIN(int nChnl, const ZCAN_LIN_SUBSCIBE_CFG &zLINSubscibe, PacketDataLIN& pktLIN)
{
	if (zLINSubscibe.chkSumMode > AUTOMATIC || zLINSubscibe.ID > MAX_LIN_ID_COUNT || zLINSubscibe.dataLen == 0)
	{
		return;
	}
	if (zLINSubscibe.dataLen > MAX_LIN_DLC && zLINSubscibe.dataLen < AUTO_LIN_DLC)
	{
		return;
	}
	memset(&pktLIN, 0, sizeof(pktLIN));
	pktLIN.nChnl = nChnl;
	pktLIN.stMsgID.nRawID = zLINSubscibe.ID;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bSendMode = LIN_SEND_SUBSCRIBE;
	pktLIN.frameInfo.linInfo.linTxInfo.unionLinTx.bChkSumMode = zLINSubscibe.chkSumMode;
	pktLIN.nDataLen = zLINSubscibe.dataLen;
}
...
Packet CPacketEncoder::BuildPacketRequest(uint8_t nSeq, const PacketDataRequest& request)
{
    ...
    uint8_t tmp[2];
    tmp[0] = request.nTID;
    tmp[1] = request.nRequest;
    PacketAddData(packet, tmp, sizeof(tmp));
    PacketAddData(packet, request.vecData.data(), request.vecData.size());
    PacketAddCRC(packet);
    return packet;
}

Packet CPacketEncoder::BuildPacketRequestEx(uint8_t nSeq, const PacketDataRequest& request)
{
    Packet packet;
    PacketTypeRequestResponseParam pktParam;
    pktParam.rawValue = 0;
    pktParam.unionValue.bResponse = 0;  // 0: request
    pktParam.unionValue.nSeq = nSeq;
    USHORT len = request.vecData.size() + 5; // 5Byte : tid + request
    PacketHead head{ PACKET_HEADER, PACKET_TYPE_REQUEST_RESPONSE_EX, pktParam.rawValue, 0, len };
    PacketAddHead(packet, head);
    uint8_t tmp[5];
    uint32_t tid = cc_htonl(request.nTID);
    memcpy(&tmp[0], &tid, 4);
    tmp[4] = request.nRequest;
    PacketAddData(packet, tmp, sizeof(tmp));
    PacketAddData(packet, request.vecData.data(), request.vecData.size());
    PacketAddCRC(packet);
    return packet;
...
Packet CPacketEncoder::BuildPacketEmulateResponse(uint8_t nSeq, const PacketDataRequest& requestData)
{
    ...
    PacketAddHead(packet, head);
    uint8_t tmp[2];
    tmp[0] = responseData.nTID;
    tmp[1] = responseData.nResult;
    PacketAddData(packet, tmp, sizeof(tmp));
...
Packet CPacketEncoder::BuildPacketEmulateResponseEx(uint8_t nSeq, const PacketDataRequest& requestData)
{
    Packet packet;
    PacketTypeRequestResponseParam pktParam;
    pktParam.rawValue = 0;
    ...
    USHORT len = responseData.vecData.size() + 5; // 5Byte : tid + result
    PacketHead head{ PACKET_HEADER, PACKET_TYPE_REQUEST_RESPONSE_EX, pktParam.rawValue, 0, len };
    PacketAddHead(packet, head);
    uint8_t tmp[5];
    uint32_t tid = cc_htonl(requestData.nTID);
    memcpy(&tmp[0], &tid, 4);
    tmp[1] = responseData.nResult;
    PacketAddData(packet, tmp, sizeof(tmp));
    PacketAddData(packet, responseData.vecData.data(), responseData.vecData.size());
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl, const PZCAN_LIN_MSG pSend, ULONG len)
{
    ...
    while (len)
    {
        ...
        for (ULONG i = 0; i<cnt; i++)
        {
            CPacketUtil::ZLIN2PacketLIN(nChnl, pSend[doneCnt + i].data.zcanLINData, pktFrame);
            PacketAddData(packet, pktFrame);
        }
        PacketAddCRC(packet);
        vecPackets.push_back(packet);
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl, const PZCAN_LIN_SUBSCIBE_CFG pSend, ULONG len)
{
	VecPackets vecPackets;
	Packet packet;
	PacketDataLIN pktFrame;
...
VecPackets CPacketEncoder::BuildPacketLIN(int nChnl, const PZCAN_LIN_PUBLISH_CFG pSend, ULONG len)
{
	VecPackets vecPackets;
	Packet packet;
	PacketDataLIN pktFrame;
...
void CPacketEncoder::PacketAddData(Packet& packet, const PacketDataLIN& pktLIN)
{
	...
	netFrame.frameInfo.linInfoRawVal = cc_htons(pktLIN.frameInfo.linInfoRawVal);
