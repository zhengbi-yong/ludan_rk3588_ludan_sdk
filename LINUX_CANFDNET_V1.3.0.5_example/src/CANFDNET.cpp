...
//2022.10.24新加
UINT FUNC_CALL ZCAN_SetLINSubscribe(CHANNEL_HANDLE channel_handle, PZCAN_LIN_SUBSCIBE_CFG pSend, UINT nMsgCount)
{
	CAN_HANDLE hCANHandle(channel_handle);
	if (hCANHandle.IsInvalid())
	{
...
UINT FUNC_CALL ZCAN_SetLINPublish(CHANNEL_HANDLE channel_handle, PZCAN_LIN_PUBLISH_CFG pSend, UINT nMsgCount)
{
	CAN_HANDLE hCANHandle(channel_handle);
	if (hCANHandle.IsInvalid())
	{
...
ZCAN_RET_STATUS FUNC_CALL ZCAN_UDS_ControlEX(DEVICE_HANDLE device_handle, ZCAN_UDS_DATA_DEF dataType, ZCAN_UDS_CTRL_REQ *ctrl, ZCAN_UDS_CTRL_RESP* resp)
{
	CAN_HANDLE hDeviceHandle(device_handle);
	if (hDeviceHandle.IsInvalid()) {
		return STATUS_ERR;
