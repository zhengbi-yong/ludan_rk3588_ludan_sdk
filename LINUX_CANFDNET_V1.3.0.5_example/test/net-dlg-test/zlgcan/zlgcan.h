...
//动态配置 持久配置 BEGIN
#define	ZCAN_DYNAMIC_CONFIG_DEVNAME "DYNAMIC_CONFIG_DEVNAME"// 参数类型:string 设备名，最长为32字节（包括’\0’），CANFDNET - 200U默认值为“CANFDNET - 200U”，CANFDNET - 100MINI默认值为“CANFDNET - 100MINI”
//CAN的通道配置信息(CAN%d需进行格式化声明通道 范围是0-7)
#define	ZCAN_DYNAMIC_CONFIG_CAN_ENABLE "DYNAMIC_CONFIG_CAN%d_ENABLE"// 参数类型:int 通道使能；1：使能，0：失能；CANFDNET系列产品通道默认使能。
#define	ZCAN_DYNAMIC_CONFIG_CAN_TYPE "DYNAMIC_CONFIG_CAN%d_TYPE"// 参数类型:int 控制器类型：0：CAN；1：ISO CANFD；2：Non - ISO CANFD。
#define	ZCAN_DYNAMIC_CONFIG_CAN_MODE "DYNAMIC_CONFIG_CAN%d_MODE"// 参数类型:int 工作模式，默认正常模式；0：正常模式；1：只听模式。
#define	ZCAN_DYNAMIC_CONFIG_CAN_CLK "DYNAMIC_CONFIG_CAN%d_CLK"// 参数类型:int 控制器时钟，单位MHz;
#define	ZCAN_DYNAMIC_CONFIG_CAN_TXATTEMPTS "DYNAMIC_CONFIG_CAN%d_TXATTEMPTS"// 参数类型:int 发送失败是否重传：0：发送失败不重传1：发送失败重传，直到总线关闭（CANFDNET - 100 / 200无此项配置）
#define	ZCAN_DYNAMIC_CONFIG_CAN_NOMINALBAUD "DYNAMIC_CONFIG_CAN%d_NOMINALBAUD"//参数类型:int CAN波特率或CANFD仲裁域波特率；
#define	ZCAN_DYNAMIC_CONFIG_CAN_DATABAUD "DYNAMIC_CONFIG_CAN%d_DATABAUD"//参数类型:int CANFD数据域波特率；
#define	ZCAN_DYNAMIC_CONFIG_CAN_USERES "DYNAMIC_CONFIG_CAN%d_USERES"//参数类型:int 终端电阻开关；0：关闭；1：打开。
#define	ZCAN_DYNAMIC_CONFIG_CAN_SNDCFG_INTERVAL "DYNAMIC_CONFIG_CAN%d_SNDCFG_INTERVAL"// 参数类型:int 报文发送间隔，0~255ms
#define	ZCAN_DYNAMIC_CONFIG_CAN_SNDCFG_BUFSIZE "DYNAMIC_CONFIG_CAN%d_SNDCFG_BUFSIZE"// 参数类型:int 发送缓冲区，10~1000，单位10帧
#define	ZCAN_DYNAMIC_CONFIG_CAN_SNDCFG_BUFMODE "DYNAMIC_CONFIG_CAN%d_SNDCFG_BUFMODE"// 参数类型:int 发送缓冲区策略；0：满丢新数据，1：满丢旧数据
#define	ZCAN_DYNAMIC_CONFIG_CAN_BUSRATIO_ENABLE "DYNAMIC_CONFIG_CAN%d_BUSRATIO_ENABLE"// 参数类型:int 总线利用率使能，使能后，将周期发送总线利用率到设定的TCP/UDP连接。1:使能，0：失能
#define	ZCAN_DYNAMIC_CONFIG_CAN_BUSRATIO_PERIOD "DYNAMIC_CONFIG_CAN%d_BUSRATIO_PERIOD"// 参数类型:int 总线利用率采集周期，取值200~2000ms
//动态配置 持久配置 END
