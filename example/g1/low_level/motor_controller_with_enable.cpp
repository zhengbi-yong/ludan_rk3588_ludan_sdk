...
private:
    ...
    struct MotorEnableMsg {
        int motor_id;    // 电机ID (0-29)
        uint8_t command;  // 使能指令: 0=禁使能, 1=使能, 其他=自定义
    };

    // ROS2 components
    rclcpp::Subscription<xixilowcmd::msg::LowCmd>::SharedPtr lowcmd_subscriber_;
    rclcpp::Subscription<xixilowcmd::msg::MotorCmd>::SharedPtr enable_subscriber_;  // 使用MotorCmd消息传递使能指令
    ...
    bool motor_cmd_enabled_ = false;  // 默认禁用电机命令发送，通过--enable-motor-cmd参数开启
    ...
    // 使能命令优先级控制
    std::atomic<bool> enable_command_active_{false};  // 使能命令正在执行时，暂停其他CAN数据
    std::atomic<bool> can_send_started_{false};      // 控制CAN发送线程是否已启动（默认false，等待信号）
...
public:
    ROS2_to_TCP_Bridge(bool verbose = false, bool motor_cmd_enabled = false) : Node("ros2_to_tcp_bridge") {
        ...
        if (motor_cmd_enabled_) {
            std::cout << "  Motor command sending: ENABLED" << std::endl;
        } else {
            std::cout << "  Motor command sending: DISABLED (use --enable-motor-cmd to enable)" << std::endl;
        }
    }
    ...
    void InitializeROS2() {
        ...
        // Create enable command subscriber (using MotorCmd message)
        ...
        std::cout << "✓ Motor enable subscriber created on topic: /motor_enable (MotorCmd format)" << std::endl;
    ...
    // 使能指令处理函数 (使用MotorCmd消息)
    void EnableCommandHandler(const xixilowcmd::msg::MotorCmd::SharedPtr msg) {
        std::cout << "\n📨 收到使能指令消息! MotorCmd: id=" << static_cast<int>(msg->id) << ", q=" << msg->q << std::endl;
        ...
        if (motor_id < 0 || motor_id > 29) {
            std::cerr << "❌ 无效的电机ID: " << motor_id << " (范围: 0-29)" << std::endl;
            return;
        }

        // 检查是否有使能指令 (q值非零)
        if (command_value == 0) {
            std::cout << "⚠️  收到使能指令但q=0，忽略" << std::endl;
            return;
        }

        // 设置使能命令激活标志，暂停普通CAN数据发送
        {
        ...
        // 清除使能命令激活标志，并启动500Hz发送
        {
            std::unique_lock<std::mutex> lock(pause_mutex_);
            enable_command_active_ = false;
            can_send_started_ = true;  // 启动500Hz发送线程
        }
        // 通知等待的线程继续执行
    ...
    bool InitializeZhilgongConnection() {
        ...
        std::cout << "🔗 连接周立功设备: " << zhilgong_config_.remote_ip << ":" << zhilgong_config_.remote_port << std::endl;
        ...
        memset(&server_addr, 0, sizeof(server_addr));
        server_addr.sin_family = AF_INET;
        server_addr.sin_port = htons(zhilgong_config_.remote_port);

        if (inet_pton(AF_INET, zhilgong_config_.remote_ip.c_str(), &server_addr.sin_addr) <= 0) {
            std::cerr << "❌ 无效的IP地址: " << zhilgong_config_.remote_ip << std::endl;
            close(zhilgong_config_.socket_fd);
            return false;
        }

        // 连接到周立功设备
        if (connect(zhilgong_config_.socket_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
            std::cerr << "❌ 连接周立功设备失败: " << strerror(errno) << std::endl;
            close(zhilgong_config_.socket_fd);
            return false;
        }

        zhilgong_config_.initialized = true;
        std::cout << "✅ 周立功设备连接成功" << std::endl;
        return true;
    }
    ...
    void SendEnableCommandToZhilgong(int motor_id) {
        ...
        int can_id = get_can_id_for_motor(motor_id);
        if (can_id <= 0) {
            std::cerr << "❌ 无效的电机ID: " << motor_id << std::endl;
            return;
        }

        // 创建使能CAN frame: FF FF FF FF FF FF FF FC
        uint8_t enable_frame[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC};

        std::cout << "\n📤 发送使能指令到周立功设备:" << std::endl;
        std::cout << "   Motor ID: " << motor_id << std::endl;
        std::cout << "   CAN ID:   0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ');
        std::cout << " (" << can_id << ")" << std::endl;
        std::cout << "   命令类型: 使能 (ENABLE)" << std::endl;
        std::cout << "   CAN Package:" << std::endl;
        std::cout << "     [CAN_ID: 0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ') << "]" << std::endl;
        std::cout << "     [DLC:    " << static_cast<int>(sizeof(enable_frame)) << "]" << std::endl;
        std::cout << "     [DATA:   ";
        for (size_t i = 0; i < sizeof(enable_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(enable_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(enable_frame) - 1) std::cout << " ";
        }
        std::cout << "]" << std::endl;
        std::cout << "   完整包: ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(can_id) << std::dec << std::setfill(' ') << " ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(sizeof(enable_frame)) << std::dec << std::setfill(' ') << " | ";
        for (size_t i = 0; i < sizeof(enable_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(enable_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(enable_frame) - 1) std::cout << " ";
        }
        ...
        SendCanFrameToZhilgong(can_id, enable_frame, sizeof(enable_frame));
        std::cout << "✅ 使能指令已发送" << std::endl;
    }
    ...
    void SendDisableCommandToZhilgong(int motor_id) {
        ...
        // 创建禁使能CAN frame: FF FF FF FF FF FF FF FD
        uint8_t disable_frame[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFD};

        std::cout << "\n📤 发送禁使能指令到周立功设备:" << std::endl;
        std::cout << "   Motor ID: " << motor_id << std::endl;
        std::cout << "   CAN ID:   0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ');
        std::cout << " (" << can_id << ")" << std::endl;
        std::cout << "   命令类型: 禁使能 (DISABLE)" << std::endl;
        std::cout << "   CAN Package:" << std::endl;
        std::cout << "     [CAN_ID: 0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ') << "]" << std::endl;
        std::cout << "     [DLC:    " << static_cast<int>(sizeof(disable_frame)) << "]" << std::endl;
        std::cout << "     [DATA:   ";
        for (size_t i = 0; i < sizeof(disable_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(disable_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(disable_frame) - 1) std::cout << " ";
        }
        std::cout << "]" << std::endl;
        std::cout << "   完整包: ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(can_id) << std::dec << std::setfill(' ') << " ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(sizeof(disable_frame)) << std::dec << std::setfill(' ') << " | ";
        for (size_t i = 0; i < sizeof(disable_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(disable_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(disable_frame) - 1) std::cout << " ";
        }
        ...
        SendCanFrameToZhilgong(can_id, disable_frame, sizeof(disable_frame));
        std::cout << "✅ 禁使能指令已发送" << std::endl;
    }
    ...
    void SendCustomEnableCommandToZhilgong(int motor_id, uint8_t command) {
        ...
        // 创建自定义CAN frame: FF FF FF FF FF FF FF [command]
        uint8_t custom_frame[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, command};

        std::cout << "\n📤 发送自定义使能指令到周立功设备:" << std::endl;
        std::cout << "   Motor ID: " << motor_id << std::endl;
        std::cout << "   CAN ID:   0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ');
        std::cout << " (" << can_id << ")" << std::endl;
        std::cout << "   命令类型: 自定义指令 (0x" << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(command) << std::dec << std::setfill(' ') << ")" << std::endl;
        std::cout << "   CAN Package:" << std::endl;
        std::cout << "     [CAN_ID: 0x" << std::hex << std::setfill('0') << std::setw(2) << can_id << std::dec << std::setfill(' ') << "]" << std::endl;
        std::cout << "     [DLC:    " << static_cast<int>(sizeof(custom_frame)) << "]" << std::endl;
        std::cout << "     [DATA:   ";
        for (size_t i = 0; i < sizeof(custom_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(custom_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(custom_frame) - 1) std::cout << " ";
        }
        std::cout << "]" << std::endl;
        std::cout << "   完整包: ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(can_id) << std::dec << std::setfill(' ') << " ";
        std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(sizeof(custom_frame)) << std::dec << std::setfill(' ') << " | ";
        for (size_t i = 0; i < sizeof(custom_frame); i++) {
            std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(custom_frame[i]) << std::dec << std::setfill(' ');
            if (i < sizeof(custom_frame) - 1) std::cout << " ";
        }
        ...
        SendCanFrameToZhilgong(can_id, custom_frame, sizeof(custom_frame));
        std::cout << "✅ 自定义使能指令已发送" << std::endl;
    }

    // 发送CAN frame到周立功设备
    void SendCanFrameToZhilgong(int can_id, const uint8_t* data, size_t data_len) {
        if (!zhilgong_config_.initialized || zhilgong_config_.socket_fd < 0) {
            std::cerr << "❌ 周立功设备未连接" << std::endl;
            return;
        }

        // 创建CAN frame包 (假设包格式: [CAN_ID(1 byte) | DLC(1 byte) | DATA(8 bytes)])
        struct CanFramePacket {
            uint8_t can_id;
            uint8_t dlc;
            uint8_t data[8];
        } packet;

        packet.can_id = static_cast<uint8_t>(can_id & 0xFF);
        packet.dlc = static_cast<uint8_t>(data_len);
        memcpy(packet.data, data, std::min(data_len, sizeof(packet.data)));

        // 发送数据
        ssize_t sent = send(zhilgong_config_.socket_fd, &packet, sizeof(packet), 0);
        if (sent < 0) {
            std::cerr << "❌ 发送CAN frame失败: " << strerror(errno) << std::endl;
        } else if (sent < static_cast<ssize_t>(sizeof(packet))) {
            std::cerr << "⚠️  部分数据发送: " << sent << "/" << sizeof(packet) << " bytes" << std::endl;
        } else {
            if (verbose_logging_) {
                std::cout << "📨 CAN frame已发送 - ID: 0x" << std::hex << static_cast<int>(packet.can_id)
                          << ", 数据: ";
    ...
    // 500Hz TCP发送线程
    void TCPSendThread() {
        std::cout << "🔄 500Hz TCP发送线程启动，等待启动信号..." << std::endl;
    ...
    void Run() {
        ...
        if (motor_cmd_enabled_) {
            ...
            std::cout << "⚠️  电机命令发送已禁用 (使用 --enable-motor-cmd 参数启用)" << std::endl;
        }
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: motor_controller <tcp_endpoint> [options]" << std::endl;
        ...
        std::cout << "         ./motor_controller 192.168.1.100:8888 --enable-motor-cmd  # Enable motor command sending" << std::endl;
        std::cout << "\nOptions:" << std::endl;
        std::cout << "  -v                 Enable verbose logging (show all messages)" << std::endl;
        std::cout << "  --enable-motor-cmd Enable motor command sending (default: DISABLED for safety)" << std::endl;
    ...
    std::cout << "Motor Command: " << (motor_cmd_enabled ? "ENABLED" : "DISABLED (use --enable-motor-cmd to enable)") << std::endl;
